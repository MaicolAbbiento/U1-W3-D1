...
*/
const stringamodificata = (string1, string2) => {
  let firststring = string1.slice(0, 2)
  let secondstring = string2.slice(-3)
...
*/
function casualArray() {
  let arrayCasuale = []

  for (let i = 0; i < 10; i++) {
    let randomNumber = Math.round(Math.random() * 100)
    arrayCasuale.push(randomNumber)
  }
...
const stringLength = function stringId(string) {
  const lengthsArray = string.map(function (string) {
...
*/
const nameOfFIlm = movies.map((nameF) => nameF.Title)
console.log(nameOfFIlm)

/* ESERCIZIO 12 (filter)
  Scrivi una funzione per ottenere dall'array fornito solamente i film usciti nel millennio corrente.
*/
const recentFilm = movies.filter((movies) => Number(movies.Year) > 1999)
console.log(recentFilm)
/* ESERCIZIO 13 (reduce)
  Scrivi una funzione per calcolare la somma di tutti gli anni in cui sono stati prodotti i film contenuti nell'array fornito.
*/
const AddictionYear = movies.reduce(
  (tot, number) => tot + Number(number.Year),
  0
)
console.log(AddictionYear)
